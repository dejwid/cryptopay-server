  ...
  const invoicesWithPayments = await Promise.all(
    invoices.map(async (invoice) => {
      if (invoice.paidAt || !invoice.coinAmount10pow10) {
        return {
          ...invoice,
          receivedAmount10pow10: null as number | null,
          paymentPercentage: null as number | null,
          paymentAddress: null as string | null,
          paymentReceivedAt: null as Date | null,
        };
      }
      ...
      if (address && address.busyFrom) {
        ...
        // Get the most recent balance change for timestamp
        const mostRecent = balanceChanges.length > 0 ? balanceChanges[0] : null;
        
        if (totalChange > 0) {
          receivedAmount10pow10 = totalChange;
          paymentReceivedAt = mostRecent?.createdAt || null;
...
export default async function DashboardInvoices() {
  ...
  const paid = invoicesWithPayments
    .filter(i => !!i.paidAt)
    .sort((a, b) => new Date(b.paidAt!).getTime() - new Date(a.paidAt!).getTime());

  const unpaidWithPartialPayment = invoicesWithPayments
    .filter(i => !i.paidAt && i.paymentPercentage !== null && i.paymentPercentage > 0)
    .sort((a, b) => (b.paymentPercentage || 0) - (a.paymentPercentage || 0));
    ...
    <>
      <Tabs.Root defaultValue="all">
        <Tabs.List>
          ...
          <Tabs.Trigger value="paid">Paid ({paid.length}, ${sum(paid.map(i => i.usdAmountCents / 100))})</Tabs.Trigger>
